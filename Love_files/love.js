(function(window){
    ...
    Seed.prototype = {
        ...
        drawText: async function() {
            ...
            ctx.moveTo(0, 0);
            ctx.scale(0.75, 0.75);
            await this.loadFont('Dancing Script');
            ctx.font = "bold 16px 'Dancing Script'"; 
            ctx.fillText("Công chúa click vào cí nì nhaaa", 23, 16);
            ctx.restore();
        },
        clear: function() {
            var ctx = this.tree.ctx, cirle = this.cirle;
        ...
        hover: function(x, y) {
            ...
            var width = 40; // Adjust this value to increase or decrease the hover area
            var height = 40; // Adjust this value to increase or decrease the hover area
            var imageData = ctx.getImageData(x - width / 2, y - height / 2, width, height);
